...
- Requires verification before claims: run the app, interact with it, inspect the console, and fix errors/shifts/overflow rather than asserting something works. Confidence: 0.8
...
- Reports problems as terse symptom statements (e.g. "i cannot signin or signup", or pasting an error string verbatim with no context) and expects the agent to diagnose the root cause itself — reproducing the failure first rather than asking for steps or guessing. Confidence: 0.7
